...
- **parents** (served via `lp=parents` from the Parenting Belief Score landing page) — a consumer register for parents of teenagers, college-bound children, and adult children. **The one rule above every other rule in this vertical: the score examines the PARENT, never the child.** The child is context and is never assessed, described, diagnosed, characterized, or predicted - not their motivation, capability, readiness, feelings, state, or choices. Nothing may promise or imply any change in the child (more contact, better performance, greater independence, different decisions), and no action in the report may require the child to respond or cooperate; the score prompt's decisive test for `decisionReadiness` is whether the Q5 week is available to the parent alone. Third parties are never characterized either - a co-parent, teacher, or the child's friend may be acknowledged and never blamed. Other hard rules from the source docs: belief is NEVER the sole cause of a parenting outcome (development, temperament, health, school, money, history and circumstance stay real); a genuine practical concern is precision, not resistance, and is never scored down; the goal is never detachment - nobody is asked to care less, ignore a real risk, or lower a standard; never "controlling", "anxious", "helicopter parent", "enmeshed", "codependent", "toxic", "let go", or any clinical vocabulary; no invented statistics and no manufactured urgency (the landing page deliberately claims no deadline). The public mechanism is unnamed on purpose (the VSL excludes the internal name) and has five parts in order - the repeated parenting moment, the meaning entering it, the familiar response, the reinforcing loop, the moment to notice - which the five beats are built around. The entry copy states the question count (five, which the funnel enforces) and the completion time, **"About 10 minutes"** - the spec shipped with `[verified time]` unfilled, but that item has since been closed and the landing page publishes the measured figure under every CTA. Keep the funnel's number identical to the landing page's; if the measurement is revised, both move together. `ctaLabel` is `"Get My Free Parenting Belief Score"`, matching the landing page's actual buttons rather than the spec's approved `"Get Your Free ..."` - the product name is identical either way, and this is what a visitor reads immediately before arriving.
